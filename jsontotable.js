...
function ConvertJsonToTable(parsedJson, tableId, tableClassName, linkid,linkTextid,linkattach,linkname) {
    
    //Patterns for links and NULL value
    var linkText=[];
    if(linkTextid!=null && linkid!=null && linkattach!=null && linkname!=null)
    {
        for(i=0;i<parsedJson.length;i++)
        {    

            var temp=parsedJson[i][linkid];
            var temp1=parsedJson[i][linkTextid];
            var fullLink=linkname+'/'+temp+'/'+temp1;
            var tempforatt=parsedJson[i][linkattach];
            parsedJson[i][linkattach]=fullLink;
            //delete parsedJson[i][linkTextid];
            linkText[i]=tempforatt;
        }
    }
    ...
    if(linkText.length!=0)
    {
            ...
            {
            link[i] = linkText[i] ? '<a href="{0}">' + linkText[i] + '</a>' :'<a href="{0}">{0}</a>';
            }
    }  
    else
    {
                ...
                for (i = 0; i < parsedJson.length; i++) {
                    for (j = 0; j < headers.length; j++) {
                        ...
                        if (isUrl) // If value is URL we auto-create a link
                        {
                            if(linkText.length!=0)
                            {    
                            tbCon += tdRow.format(link[i].format(value));
                            }
                            else
                            {
